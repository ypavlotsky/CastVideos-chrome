 ...
 *    IDLE: Default state indicating that Cast extension is installed,
 *        but showing no current activity
 *    ACTIVE: Shown when Chrome has one or more local activities running
 *        on a receiver
 *    WARNING: Shown when the device is actively being used, but when one or
 *        more issues have occurred
...
var CastPlayer = function() {
  ...
  this.autoplay = true;
  /**
   * @type {chrome.cast.Session} object
   */
  this.session = null;
  /**
  ...
  this.timer = null;
  /**
   * @type {boolean} A boolean to stop timer update of progress when
   *     triggered by media status event
   */
  this.progressFlag = true;
  /**
   * @const {number} A number in milliseconds for minimal progress update
   */
  this.timerStep = 1000;
...
/**
 * @param {chrome.cast.media.PlayerState} playerState
 * @return {PLAYER_STATE}
 */
CastPlayer.getPlayerState = function(playerState) {
  switch (playerState) {
 ...
 * Initializes the API. Note that either successCallback and errorCallback will
 * be invoked once the API has finished initialization. The sessionListener and
 * receiverListener may be invoked at any time afterwards, and possibly more
 * than once.
 */
CastPlayer.prototype.initializeCastPlayer = function() {
  ...
  chrome.cast.initialize(
      apiConfig, this.onInitSuccess.bind(this), this.errorHandler);
 ...
 * @param {!chrome.cast.Session} session A new session
 */
CastPlayer.prototype.sessionListener = function(session) {
  this.session = session;
  this.deviceState = DEVICE_STATE.ACTIVE;
  if (this.session.media[0]) {
    this.onMediaDiscovered(false, this.session.media[0]);
  }
  else {
 ...
 * By default, the SessionRequest passed to the API at initialization time is
 * used; this may be overridden by passing a different session request in
 * opt_sessionRequest.
 */
CastPlayer.prototype.launchApp = function() {
  console.log('launching app...');
  chrome.cast.requestSession(
      this.onRequestSessionSuccess.bind(this), this.onLaunchError.bind(this));
  if (this.timer) {
    clearInterval(this.timer);
  }
};
 ...
 */
CastPlayer.prototype.onRequestSessionSuccess = function(session) {
...
CastPlayer.prototype.onLaunchError = function(error) {
  var errorMessage = error ?
      ...
      'null';
  console.log('launch error: ' + errorMessage);
...
CastPlayer.prototype.loadMedia = function(mediaIndex) {
  ...
  this.session.loadMedia(request,
      this.onMediaDiscovered.bind(this, true),
 ...
 * @param {boolean} isNewMediaLoaded
 * @param {chrome.cast.media.Media} mediaSession
 */
CastPlayer.prototype.onMediaDiscovered = function(
    isNewMediaLoaded, mediaSession) {
  var how = isNewMediaLoaded ? 'loadMedia' : 'activeSession';
  console.log(
      'new media session ID:' + mediaSession.mediaSessionId + ' (' + how + ')');
  this.currentMediaSession = mediaSession;
  if (isNewMediaLoaded) {
    this.castPlayerState = this.autoplay ?
        PLAYER_STATE.PLAYING : PLAYER_STATE.LOADED;
  } else {
    this.castPlayerState = CastPlayer.getPlayerState(
        this.session.media[0].playerState);
  ...
  this.currentMediaDuration = this.currentMediaSession.media.duration ?
      this.currentMediaSession.media.duration : -1;
 ...
 * @param {chrome.cast.Error} error
 */
CastPlayer.prototype.onLoadMediaError = function(error) {
  var errorMessage = error ?
      'code = ' + error.code + ', description = ' + error.description :
      'null';
 ...
 * @param {boolean} isMediaObjectAlive
 */
CastPlayer.prototype.onMediaStatusUpdate = function(isMediaObjectAlive) {
  if (!isMediaObjectAlive) {
    this.currentMediaTime = 0;
    this.castPlayerState = PLAYER_STATE.IDLE;
  }
...
CastPlayer.prototype.playMediaLocally = function(currentTime) {
  ...
  if (this.localPlayerState != PLAYER_STATE.PLAYING &&
    ...
    this.localPlayer.src =
        this.mediaContents[this.currentMediaIndex]['sources'][0];
    this.localPlayer.load();
    this.localPlayer.addEventListener(
        'loadeddata', this.onMediaLoadedLocally.bind(this, currentTime));
  }
  else {
  ...
  if (this.castPlayerState == PLAYER_STATE.PLAYING) {
    ...
    this.currentMediaSession.pause(null,
        this.mediaCommandSuccessCallback.bind(
            this, 'paused ' + this.currentMediaSession.sessionId),
        this.errorHandler);
    this.updateMediaControlUI();
    this.updateDisplayMessage();
    clearInterval(this.timer);
...
CastPlayer.prototype.stopMedia = function() {
  ...
  this.currentMediaSession.stop(null,
      this.mediaCommandSuccessCallback.bind(
          this, 'stopped ' + this.currentMediaSession.sessionId),
      this.errorHandler);
...
CastPlayer.prototype.setReceiverVolume = function(mute, event) {
  ...
  if (event.currentTarget.id == 'audio_bg_track' ||
      event.currentTarget.id == 'audio_bg_level') {
  ...
  if (!mute) {
    this.session.setReceiverVolumeLevel(this.currentVolume,
        this.mediaCommandSuccessCallback.bind(this,
            'setReceiveVolumneLevel() succeeded'),
        this.errorHandler);
  }
  else {
    this.session.setReceiverMuted(true,
        this.mediaCommandSuccessCallback.bind(this,
            'setReceiverMuted() succeeded'),
        this.errorHandler);
  }
  this.updateMediaControlUI();
};
...
CastPlayer.prototype.seekMedia = function(event) {
  ...
  if (event.currentTarget.id == 'progress_indicator') {
    var curr = this.currentMediaTime +
 ...
 * @param {string} mediaCommand
 */
CastPlayer.prototype.mediaCommandSuccessCallback = function(mediaCommand) {
  if (mediaCommand) {
    console.log('mediaCommandSuccessCallback: ' + mediaCommand);
  }
};
...
CastPlayer.prototype.updateProgressBar = function() {
  var p = document.getElementById('progress');
  var pi = document.getElementById('progress_indicator');
  var mediaSession = this.currentMediaSession;
  if (mediaSession.playerState === chrome.cast.media.PlayerState.IDLE) {
    ...
    p.style.width =
        Math.ceil(PROGRESS_BAR_WIDTH * mediaSession.currentTime /
            mediaSession.media.duration + 1) +
        'px';
    this.progressFlag = false;
    setTimeout(this.setProgressFlag.bind(this), 1000);
    var pp = Math.ceil(PROGRESS_BAR_WIDTH * mediaSession.currentTime /
...
CastPlayer.prototype.selectMediaUpdateUI = function(mediaIndex) {
  document.getElementById('video_image').src =
      MEDIA_SOURCE_ROOT + this.mediaContents[mediaIndex]['thumb'];
  document.getElementById('progress').style.width = '0px';
  document.getElementById('media_title').innerHTML =
      this.mediaContents[mediaIndex]['title'];
  document.getElementById('media_subtitle').innerHTML =
      this.mediaContents[mediaIndex]['subtitle'];
  document.getElementById('media_desc').innerHTML =
      this.mediaContents[mediaIndex]['description'];
};
...
CastPlayer.prototype.initializeUI = function() {
  ...
  document.getElementById('casticonidle').addEventListener(
      'click', this.launchApp.bind(this));
  document.getElementById('casticonactive').addEventListener(
      'click', this.stopApp.bind(this));
  ...
  document.getElementById('audio_on').addEventListener(
      'click', this.muteMedia.bind(this));
  ...
  document.getElementById('audio_bg_track').addEventListener(
      'mouseover', this.showVolumeSlider.bind(this));
  document.getElementById('audio_bg_level').addEventListener(
      'click', this.setReceiverVolume.bind(this, false));
  document.getElementById('audio_bg_track').addEventListener(
      'click', this.setReceiverVolume.bind(this, false));
  document.getElementById('audio_bg').addEventListener(
  ...
  document.addEventListener(
      'fullscreenchange', this.changeHandler.bind(this), false);
  document.addEventListener(
      'webkitfullscreenchange', this.changeHandler.bind(this), false);

  // enable play/pause buttons
  document.getElementById('play').addEventListener(
      'click', this.playMedia.bind(this));
  document.getElementById('pause').addEventListener(
      'click', this.pauseMedia.bind(this));
  ...
  'categories': [{
    ...
    'videos': [{
      'description': 'Big Buck Bunny tells the story of a giant rabbit with ' +
          ...
          'bunny anymore! In the typical cartoon tradition he prepares the ' +
          'nasty rodents a comical revenge.\n\nLicensed under the Creative ' +
      ...
      'description': 'HBO GO now works with Chromecast -- the easiest way to' +
          'enjoy online video on your TV. For when you want to settle into ' +
          'your Iron Throne to watch the latest episodes. For $35.\nLearn ' +
          'how to use Chromecast with HBO GO and more at ' +
      ...
      'description': 'Introducing Chromecast. The easiest way to enjoy ' +
          'online video and music on your TVâ€”for when Batman\'s escapes ' +
          'aren\'t quite big enough. For $35. Learn how to use Chromecast ' +
          'with Google Play Movies and more at google.com/chromecast.',
      'sources': [
      ...
      'description': 'Introducing Chromecast. The easiest way to enjoy ' +
          'online video and music on your TVâ€”for the times that call for ' +
          'bigger joyrides. For $35. Learn how to use Chromecast with ' +
      ...
      'description': 'Introducing Chromecast. The easiest way to enjoy ' +
          'online video and music on your TVâ€”for when you want to make ' +
          'Buster\'s big meltdowns even bigger. For $35. Learn how to use ' +
      ...
      'description': 'Sintel is an independently produced short film, ' +
          'initiated by the Blender Foundation as a means to further improve ' +
          'and validate the free/open source 3D creation suite Blender. With ' +
          'initial funding provided by 1000s of donations via the internet ' +
          'community, it has again proven to be a viable development model ' +
          'for both open 3D technology as for independent animation film.\n' +
          'This 15 minute film has been realized in the studio of the ' +
          'Amsterdam Blender Institute, by an international team of artists ' +
          'and developers. In addition to that, several crucial technical ' +
          'and creative targets have been realized online, by developers ' +
          'and artists and teams all over the world.\nwww.sintel.org',
      'sources': [
      ...
      'description': 'Smoking Tire takes the all-new Subaru Outback to the ' +
          'highest point we can find in hopes our customer-appreciation ' +
          'Balloon Launch will get some free T-shirts into the hands of our ' +
          'viewers.',
      'sources': [
      ...
      'description': 'Tears of Steel was realized with crowd-funding by ' +
          'users of the open source 3D creation tool Blender. Target was to ' +
          'improve and test a complete open and free pipeline for visual ' +
          'effects in film - and to make a compelling sci-fi film in ' +
          'Amsterdam, the Netherlands.  The film itself, and all raw ' +
          'material used for making it, have been released under the ' +
          'Creatieve Commons 3.0 Attribution license. Visit the ' +
          'tearsofsteel.org website to find out more about this, or to ' +
          'purchase the 4-DVD box with a lot of extras.  ' +
      ...
      'description': 'The Smoking Tire heads out to Adams Motorsports Park ' +
          'in Riverside, CA to test the most requested car of 2010, the ' +
          'Volkswagen GTI. Will it beat the Mazdaspeed3\'s standard-setting ' +
          'lap time? Watch and see...',
      'sources': [
      ...
      'description': 'The Smoking Tire is going on the 2010 Bullrun Live ' +
          'Rally in a 2011 Shelby GT500, and posting a video from the road ' +
          'every single day! The only place to watch them is by subscribing ' +
          'to The Smoking Tire or watching at BlackMagicShine.com',
      'sources': [
      ...
      'description': 'The Smoking Tire meets up with Chris and Jorge from ' +
          'CarsForAGrand.com to see just how far $1,000 can go when looking ' +
          'for a car.The Smoking Tire meets up with Chris and Jorge from ' +
          'CarsForAGrand.com to see just how far $1,000 can go when looking ' +
          'for a car.',
      'sources': [
